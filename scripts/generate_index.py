#!/usr/bin/env python3
"""Generate index.html listing all sleep medicine daily reports."""
import glob
import os
from datetime import datetime

html_files = sorted(glob.glob("docs/sleep-*.html"), reverse=True)

links = ""
for f in html_files[:30]:
    name = os.path.basename(f)
    date = name.replace("sleep-", "").replace(".html", "")
    try:
...
<title>Sleep Brain &middot; 睡眠醫學文獻日報</title>
<style>
  :root {{ --bg: #0d1b2a; --surface: #1b2838; --line: #2a3f55; --text: #e0e6ed; --muted: #8899aa; --accent: #00b4d8; --accent-soft: rgba(0,180,216,0.12); }}
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: linear-gradient(135deg, #0d1b2a 0%, #1b2838 40%, #162447 100%); color: var(--text); font-family: "Noto Sans TC", "PingFang TC", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; }}
  ...
  .clinic-link {{ display: block; padding: 14px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; text-decoration: none; color: var(--text); margin-bottom: 8px; transition: all 0.2s; }}
...
<div class="container">
  <div class="logo">😴</div>
  <h1>Sleep Brain</h1>
  <p class="subtitle">睡眠醫學文獻日報 &middot; 每日自動更新</p>
  <p class="count">共 {total} 期日報</p>
  <ul>{links}</ul>
  <div class="clinic-banner">
    <a href="https://www.leepsyclinic.com/" class="clinic-link" target="_blank">🏥 李政洋身心診所首頁</a>
